...
const Navbar = () => {
  ...
  const handleNotificationsOpen = useCallback(async () => {
    ...
    if (entries.length > 200) {
      entries = entries.sort((a, b) => b[1] - a[1]).slice(0, 200);
    }
              ...
              {getRoleNavLinks(user.role).map((link) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={hasIcon ? "hidden lg:inline" : "leading-none"}
                    >
                      {link.label}
                    </span>
                  </NavLink>
              ...
              <div className="md:hidden mobile-menu-container">
                <button
                  ...
                  className={`btn btn-ghost btn-sm p-3 min-w-[44px] min-h-[44px] flex items-center justify-center touch-manipulation select-none relative ${
                    isLightTheme
                      ? "text-emerald-50 hover:bg-white/10 focus-visible:ring-white/40"
                      : ""
                  }`}
                  style={{
              ...
              <div className="flex items-center gap-3">
                <div className="dropdown dropdown-end">
                  ...
                  >
                    ...
                    <Bell
                      className={`h-5 w-5 ${notifLoading ? "animate-pulse" : ""}`}
                    />
                  </div>
                  <div
                    tabIndex={0}
                ...
                <div className="dropdown dropdown-end">
                  ...
                  >
                    ...
                    <div className="hidden sm:flex flex-col leading-tight">
                      <span
                        className={`text-sm font-semibold ${isLightTheme ? "text-emerald-50" : "text-base-content"}`}
                      >
                        {user?.name || user?.email || "User"}
                      </span>
                      <span
                        className={`text-xs sm:truncate max-w-[180px] ${isLightTheme ? "text-emerald-100/80" : "text-base-content/60"}`}
                      >
                  ...
                  >
                    <li className="menu-title">
                      <span
                        className={`text-xs ${isLightTheme ? "text-emerald-600" : ""}`}
                      >
                        Account Info
                      </span>
                    </li>
                    <li>
                      <div className="flex flex-col items-start p-2 cursor-default hover:bg-transparent">
                        <span
                          className={`font-semibold text-sm ${isLightTheme ? "text-emerald-900" : ""}`}
                        >
                          {user?.name || "User"}
                        </span>
                        <span
                          className={`text-xs ${isLightTheme ? "text-emerald-700/80" : "opacity-70"}`}
                        >
                          {user?.email || "No email"}
                        </span>
                        <span
