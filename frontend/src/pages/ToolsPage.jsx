...
const ToolsPage = () => {
  ...
  const needsRepairTools = tools.filter(
    (t) => String(t.condition).toLowerCase() === "needs_repair",
  ).length;
  const retiredTools = allTools.filter(
    (t) => String(t.condition).toLowerCase() === "retired",
  ).length;
  ...
  const fetchTools = useCallback(
    async (retryCount = 0) => {
      ...
      try {
        ...
        if (shouldRetry) {
          console.log(
            `Retrying tools fetch... (attempt ${retryCount + 1}/${MAX_RETRIES})`,
          );
          setLoading(false);
    ...
    try {
      ...
      autoTable(doc, {
        body: [
          [
            ...
            {
              content: `Available: ${availableTools}`,
              styles: { textColor: [34, 197, 94] },
            }, // green
            {
              content: `Assigned: ${assignedTools}`,
              styles: { textColor: [202, 138, 4] },
            }, // yellow
            {
              content: `Needs Repair: ${needsRepairTools}`,
              styles: { textColor: [220, 38, 38] },
            }, // red
            {
              content: `Tool Types: ${uniqueTypes}`,
              styles: { textColor: [30, 41, 59] },
            },
          ],
        ],
      ...
      if (win)
        win.document.body.innerHTML =
          '<p style="font-family:sans-serif">Failed to generate PDF.</p>';
    }
  };

  const deleteTool = async (id) => {
    const ok = await Sweet.confirm(
      "Are you sure you want to retire this tool? This action will mark it as permanently out of service but keep it in records.",
    );
    if (!ok) return;
  ...
  const assignTool = async () => {
    ...
    setActionLoading(true);
    try {
      await api.post(`/tools/${assignModal.tool._id}/assign`, {
      ...
      <div className="container mx-auto p-4 sm:p-6">
        <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 sm:items-center">
            ...
            >
              <option value="">Tool Types</option>
            ...
            >
              <option value="">Tool Status</option>
            ...
            <button
              className="btn btn-primary btn-sm sm:btn-md w-full sm:w-auto sm:ml-auto"
              onClick={() => navigate("/inventory/tools/create")}
            >
        ...
        <div
          className={`rounded-lg shadow p-3 sm:p-4 mb-4 ${isLightTheme ? "bg-white border border-slate-200" : "bg-base-100 border border-gray-700/30"}`}
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-1 sm:gap-2">
            <div
              className={`rounded-md p-2 flex items-center gap-2 border hover:shadow-md transition-all duration-200 ${isLightTheme ? "bg-white border-slate-200 text-slate-700" : "bg-gradient-to-r from-slate-800 to-slate-900 border-gray-700/50 text-slate-100"}`}
            >
              <div
                className={`p-1.5 rounded-full flex-shrink-0 ${isLightTheme ? "bg-blue-100 text-blue-600" : "bg-blue-500/20 text-blue-300"}`}
              >
                <Package className="w-3 h-3" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-xs text-base-content/70 font-medium truncate">
                  Total Tools
                </div>
                <div
                  className={`font-bold text-base sm:text-lg ${isLightTheme ? "text-slate-900" : "text-blue-300"}`}
                >
                  {totalTools}
                </div>
              </div>
            ...
            <div
              className={`rounded-md p-2 flex items-center gap-2 border hover:shadow-md transition-all duration-200 ${isLightTheme ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-gradient-to-r from-green-800/20 to-green-900/30 border-green-700/30 text-emerald-300"}`}
            >
              <div
                className={`p-1.5 rounded-full flex-shrink-0 ${isLightTheme ? "bg-emerald-100 text-emerald-600" : "bg-green-500/20 text-green-200"}`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div
                  className={`font-bold text-base sm:text-lg ${isLightTheme ? "text-emerald-700" : "text-green-200"}`}
                >
                  {availableTools}
                </div>
              </div>
            ...
            <div
              className={`rounded-md p-2 flex items-center gap-2 border hover:shadow-md transition-all duration-200 ${isLightTheme ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-gradient-to-r from-amber-800/20 to-amber-900/30 border-amber-700/30 text-amber-300"}`}
            >
              <div
                className={`p-1.5 rounded-full flex-shrink-0 ${isLightTheme ? "bg-amber-100 text-amber-600" : "bg-amber-500/20 text-amber-200"}`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div
                  className={`font-bold text-base sm:text-lg ${isLightTheme ? "text-amber-700" : "text-amber-200"}`}
                >
                  {assignedTools}
                </div>
              </div>
            ...
            <div
              className={`rounded-md p-2 flex items-center gap-2 border hover:shadow-md transition-all duration-200 ${isLightTheme ? "bg-rose-50 border-rose-200 text-rose-700" : "bg-gradient-to-r from-red-800/20 to-red-900/30 border-red-700/30 text-rose-200"}`}
            >
              <div
                className={`p-1.5 rounded-full flex-shrink-0 ${isLightTheme ? "bg-rose-100 text-rose-600" : "bg-red-500/20 text-red-200"}`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div
                  className={`font-bold text-base sm:text-lg ${isLightTheme ? "text-rose-700" : "text-red-200"}`}
                >
                  {needsRepairTools}
                </div>
              </div>
            ...
            <div
              className={`rounded-md p-2 flex items-center gap-2 border hover:shadow-md transition-all duration-200 ${isLightTheme ? "bg-slate-100 border-slate-200 text-slate-700" : "bg-gradient-to-r from-gray-800/20 to-gray-900/30 border-gray-700/30 text-slate-300"}`}
            >
              <div
                className={`p-1.5 rounded-full flex-shrink-0 ${isLightTheme ? "bg-slate-200 text-slate-600" : "bg-gray-500/20 text-gray-300"}`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div
                  className={`font-bold text-base sm:text-lg ${isLightTheme ? "text-slate-700" : "text-gray-300"}`}
                >
                  {retiredTools}
                </div>
              </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                    <div>
                      ...
                      <span
                        className={
                          tool.assignedTo ? "" : "text-base-content/50"
                        }
                      >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(() => {
                      ...
                      if (isRetired) {
                        return (
                          <button
                            className="btn btn-xs btn-disabled flex-1 min-w-0"
                            disabled
                          >
                        ...
                        <div className="flex items-center gap-1">
                          <div className="w-24 flex-shrink-0">
                            {(() => {
                              const isAssigned = !!tool.assignedTo;
                              const isRepair =
                                String(tool.condition).toLowerCase() ===
                                "needs_repair";
                              const isRetired =
                                String(tool.condition).toLowerCase() ===
                                "retired";
                              ...
                              if (isRetired) {
                                return (
                                  <button
                                    className="btn btn-sm btn-disabled w-full text-xs"
                                    disabled
                                  >
                                    Retired
                                  </button>
                          ...
                          <button
                            ...
                            onClick={() =>
                              navigate(`/inventory/tools/${tool._id}`)
                            }
                          >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-base-100 rounded-xl shadow-lg p-6 w-full max-w-md">
              ...
              <div className="mb-4 text-base-content/80">
                {noteModal.note && noteModal.note.trim() ? (
                  noteModal.note
                ) : (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-base-100 rounded-xl shadow-lg p-6 w-full max-w-md">
              ...
              <div className="flex gap-2 justify-end">
                <button
                  className="btn"
                  onClick={() => setAssignModal({ open: false, tool: null })}
                >
                  Cancel
                </button>
                <button
